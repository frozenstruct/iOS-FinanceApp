...
class GraphDataSource {
    ...
    func getTimeframeData(timeFrame: TimeFrame, input data: Results<Entry> = entries, cutOff: Date) {
        ...
        switch timeFrame {
        case .day:
            dailyIncomeData = getDailyData(
                input: incomeEntries,
                inputDic: dailyIncomeData
            )
            dailyExpensesData = getDailyData(
                input: expensesEntries,
                inputDic: dailyExpensesData
            )
            
            sortedDailyIncomeData = dailyIncomeData
                .sorted { $0.key.upperBound < $1.key.lowerBound }
            sortedDailyExpensesData = dailyExpensesData
                .sorted { $0.key.upperBound < $1.key.lowerBound }
        case .week:
            weeklyIncomeData = getWeeklyData(
                input: incomeEntries,
                inputDic: weeklyIncomeData
            )
            weeklyExpensesData = getWeeklyData(
                input: expensesEntries,
                inputDic: weeklyExpensesData
            )
            
            sortedWeeklyIncomeData = weeklyIncomeData
                .sorted { $0.key < $1.key }
            sortedWeeklyExpensesData = weeklyExpensesData
                .sorted { $0.key < $1.key }
        case .month:
            monthlyIncomeData = getMonthlyData(
                input: incomeEntries,
                inputDic: monthlyIncomeData
            )
            monthlyExpensesData = getMonthlyData(
                input: expensesEntries,
                inputDic: monthlyExpensesData)
            
            sortedMonthlyIncomeData = monthlyIncomeData
                .sorted { $0.key < $1.key }
            sortedMonthlyExpensesData = monthlyExpensesData
                .sorted { $0.key < $1.key }
        case .year:
            yearlyIncomeData = getYearlyAmounts(
                input: incomeEntries,
                inputDic: yearlyIncomeData
            )
            yearlyExpensesData = getYearlyAmounts(
                input: expensesEntries,
                inputDic: yearlyExpensesData
            )
            
            sortedYearlyIncomeData = yearlyIncomeData
                .sorted { $0.key < $1.key }
            
            sortedYearlyExpensesData = sortedYearlyExpensesData
                .sorted { $0.key < $1.key }
        default:
            break
        }
    ...
    func getDailyData(input: [Entry], inputDic: [ClosedRange<Int> : Int]) -> [ClosedRange<Int> : Int] {
        ...
        for (hours, _) in res {
            res[hours] = input.filter { hours.contains($0.hour) }
                .map { (entry) -> Int in
                    if entry.amount < 0 {
    ...
    func getWeeklyData(input: [Entry], inputDic: [Int : Int]) -> [Int : Int] {
        ...
        for (day, _) in res {
            res[day] = input.filter { $0.weekDay == day }
                .map{ (entry) -> Int in
                    if entry.amount < 0 {
                        let amendedExpense: Int = entry.amount * -1
                        return amendedExpense
    ...
    func getMonthlyData(input: [Entry], inputDic: [Int : Int]) -> [Int : Int] {
        ...
        for (week, _) in res {
            res[week] = input.filter { $0.weekOfMonth == week }
                .map { (entry) -> Int in
                    if entry.amount < 0 {
                        let amendedExpense: Int = entry.amount * -1
                        return amendedExpense
    ...
    func getYearlyAmounts(input: [Entry], inputDic: [Int : Int]) -> [Int : Int] {
        ...
        for (quarter, _) in res {
            res[quarter] = input.filter { $0.quarter == quarter }
                .map { (entry) -> Int in
                    if entry.amount < 0 {
                        let amendedExpense: Int = entry.amount * -1
                        return amendedExpense
